      ...
      if (spsTradingPartnerId === 548 || spsTradingPartnerId === '548') {
        ...
        for (let i = 0; i < getLineCountSps; i++) {
          ...
          if (!!invoiceNumber && paymentCreateCheckbox == false) {
            ...
            paymentObj[invoiceNumber] = {netPaidAmt: netPaidAmt, remittanceDisc: remittanceDisc + Math.abs(adjustAmt)}
            loadSpsRecord.setSublistValue({
              sublistId: 'line',
              fieldId: 'custcol_gbs_ispaymentcreate',
              line: i,
            ...
            if (obj.status === 'paidInFull') {
              ...
              if (lineNo != -1) {
                ...
                invoiceToPayment.setCurrentSublistValue({
                  sublistId: 'apply',
                  fieldId: 'amount',
                  value: paymentObj[tranid].netPaidAmt
                })

                invoiceToPayment.setCurrentSublistValue({
                  sublistId: 'apply',
                  fieldId: 'disc',
                  value: paymentObj[tranid].remittanceDisc
                })
